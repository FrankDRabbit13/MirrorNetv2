...
function CircleCard({ circle, user, onSuggestGoal }: { circle: Circle; user: User; onSuggestGoal: () => void; }) {
...
function EcoRatingCard({ user }: { user: User; }) {
...
export default function DashboardPage() {
  ...
  const circleOrder = ["Friends", "General", "Work", "Family"];
      ...
      {circles.length > 0 || user ? (
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {sortedCircles.map((circle) => (
            <CircleCard key={circle.id} circle={circle} user={user} onSuggestGoal={handleOpenGoalDialog} />
          ))}
          <EcoRatingCard user={user} />
